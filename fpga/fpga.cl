...
__kernel void do_work(
  ...
  for(int i = 0; i<iterations; i++) {
	inner_loop: for (int p = 0; p < 16; p++) {

		u32 localIndex = startIndex + i * 16 + p;
          
		// Mine
		sha256_ctx_t ctx = miner_apply(ctx0, tail, localIndex);
